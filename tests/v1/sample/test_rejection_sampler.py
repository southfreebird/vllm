...
def create_sampling_metadata(
    ...
    min_tokens: Optional[dict[int, tuple[int, set[int]]]] = None,
) -> SamplingMetadata:
    ...
    return SamplingMetadata(
        ...
        min_tokens={} if min_tokens is None else min_tokens,
        logit_bias=[None],
...
def test_min_tokens_penalty(rejection_sampler):
    """Test rejection sampling with min tokens penalty"""
    spec_tokens = [[1, 3, 10], [10, 10, 3], [7, 15, 12]]
    output_tokens = [[1, 3, 10, 5], [10, 10, 10, 5], [7, 10, 12, 5]]
    ...
    for index in range(batch_size):
        min_tokens[index] = (NUM_OUTPUT_TOKENS, set([3, 5, 10]))
